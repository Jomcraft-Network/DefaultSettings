...
public class ButtonCheckboxSegment extends Segment {
	...
	public void customRender(int mouseX, int mouseY, float customPosX, float customPosY, float partialTicks) {
        ...
        NEX.drawRectRoundedCorners(customPosX - 2 - 3, customPosY - 2 - 3, (float) customPosX + this.width + 2 + 3, (float) customPosY + this.height + 2 + 3, color, outRad);
		...
		if (this.timer <= MathUtil.PI / 3) {
			color = 0xff282828;
			NEX.drawRectRoundedCorners(customPosX - 2 - 3 + inRad, customPosY - 2 - 3 + inRad, (float) customPosX + this.width + 2 + 3 - inRad, (float) customPosY + this.height + 2 + 3 - inRad, color, innerRadius < 0 ? 0 : innerRadius);
		}
		...
		if (this.timer > 0) {
			...
			NEX.drawLine2D_2(f, f1, f2, f3, scaleFactor, 3F, new Vec2f((float) customPosX - 2, (float) customPosY + 1.5F), new Vec2f((float) customPosX + 4 - 3, (float) customPosY + 4 + 1), new Vec2f((float) customPosX + 7 - 2.5F, (float) customPosY - 5 + 3F));
		}
