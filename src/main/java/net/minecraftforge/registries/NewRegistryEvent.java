 ...
 * {@link RegistryBuilder} and {@link #create(RegistryBuilder)}.
 */
public class NewRegistryEvent extends Event implements IModBusEvent {
	 ...
	 *         Resolving too early will return null.
	 */
	...
	void fill() {
		Map<RegistryBuilder<?>, IForgeRegistry<?>> builtRegistries = this.registries.stream().collect(Collectors.toMap(RegistryData::builder, d -> d.builder().create(), (a, b) -> a, IdentityHashMap::new));

		// builtRegistries.forEach((builder, reg) -> {
		// if (builder.getHasWrapper() &&
		// !Registry.REGISTRY.containsKey(reg.getRegistryName()))
