...
public class ExportSegment extends BakedSegment {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if (!compiled) {
			...
			NEX.drawGradient(0, 1, 72 - this.menu.offs, 2, 0xffaaaaaa, 0x00e6e6e6, 1);

			NEX.drawGradient(0, 41, 72 - this.menu.offs, 42, 0xffaaaaaa, 0x00e6e6e6, 1);
			...
			final int getRed = NEX.getRed(on);
			final int getGreen = NEX.getGreen(on);
			final int getBlue = NEX.getBlue(on);

			this.processFactor = (float) ((Math.sin(3 * this.animTimer - (MathUtil.PI / 2)) + 1) / 2);

			final float red = (NEX.getRed(off) - getRed) * (1 - processFactor);

			final float green = (NEX.getGreen(off) - getGreen) * (1 - processFactor);

			final float blue = (NEX.getBlue(off) - getBlue) * (1 - processFactor);

			on = new Color((int) (getRed + red), (int) (getGreen + green), (int) (getBlue + blue)).getRGB();

			if (inactive)
				on = NEX.darkenColor(0xff5b5b5b, darken).getRGB();

			float xPos = (72 - this.menu.offs) / 2 - 29F / 2F;

			NEX.drawRectRoundedCorners(xPos + offset * 2 - 0.5F, 20 + offset - 1, xPos + 29 - offset * 2 + 0.5F, 14 + 20 - offset - 1F, NEX.darkenColor(on, 0.5F).getRGB(), 7 - offset);

			NEX.drawRectRoundedCorners(xPos + offset * 2, 20 + offset, xPos + 29 - offset * 2, 14 + 20 - offset, on, 7 - offset);
			...
			NEX.drawCircle((float) ((28F - offset * 4) * processFactor) + xPos + offset * 2, (float) 7 + 20, radius - offset, 0, 0);
			radius = 8.5F;
			...
			if (!(percent >= 0.8F))
				fontRenderer.drawString("Export Mode:", 2F + (72 - this.menu.offs) / 2 - fontRenderer.getStringWidth("Export Mode:", 0.9F, true) / 2, 6, NEX.calcAlpha(0xffffffff, percent).getRGB(), 0.9F, true);
	...
	public boolean isSelected(int mouseX, int mouseY) {
		float offset = this.menu.offs / 20;
		float xPos = (72 - this.menu.offs) / 2 - 29F / 2F;
		return (mouseX >= this.getPosX() + xPos + offset * 2 && mouseY >= this.getPosY() + 20 + offset && mouseX < this.getPosX() + xPos + 29 - offset * 2 && mouseY < this.getPosY() + 14 + 20 - offset) || (NEX.distanceBetweenPoints((float) ((28F - offset * 4) * processFactor) + xPos + offset * 2, this.posY + 7 + 20, (float) mouseX, (float) mouseY) <= 9.4F - offset);
	}
