...
import net.jomcraft.defaultsettings.mixin.KeyMappingMixin;
...
public class FabricCoreHook implements ICoreHook {
    ...
    public KeyPlaceholder[] getKeyMappings() {
        ...
        for (int i = 0; i < mappings.length; i++) {
            keys[i] = new KeyPlaceholder(mappings[i].getName(), ((KeyMappingMixin) mappings[i]).getKey().toString(), null);
        }
        return keys;
    }
    ...
    public void setKeybind(KeyPlaceholder key, boolean init) {
        ...
        for (int i = 0; i < mappings.length; i++) {
            if (mappings[i].getName().equals(key.name)) {
                ...
                ((KeyMappingMixin) mappings[i]).setDefaultKey(container.input);
