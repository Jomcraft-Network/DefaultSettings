		...
		tpe.execute(new ThreadRunnable(source, null) {
			...
			public void run() {
				try {
					...
					source.sendFeedback(new TextComponentString(TextFormatting.RED + "Couldn't switch the export-mode"), true);
				}
			}
		});
		return 0;
			...
			public void run() {
				try {
					...
					if(somethingChanged && !argument.equals("-of")) {
						source.sendFeedback(new TextComponentString(TextFormatting.GOLD + "\n\n"), true);
						source.sendFeedback(new TextComponentString(TextFormatting.GOLD + "You seem to have updated certain config files!"), true);
						source.sendFeedback(new TextComponentString(TextFormatting.GOLD + "Users who already play your pack won't (!) receive those changes.\n"), true);
						source.sendFeedback(new TextComponentString(TextFormatting.GOLD + "If you want to ship the new configs to those players too,"), true);
						source.sendFeedback(new TextComponentString(TextFormatting.GOLD + "append the '-of' argument instead of '-o'"), true);
					}
				} catch (Exception e) {
