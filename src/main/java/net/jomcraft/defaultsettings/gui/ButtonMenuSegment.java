...
public class ButtonMenuSegment extends ButtonSegment {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		int outer = NEX.calcAlpha(0xffff8518, percent * 1.7F).getRGB();
		
		int inner = NEX.calcAlpha(0xff505050, percent * 1.7F).getRGB();
		...
		int alphaTest = MathUtil.clamp((int) ((NEX.getAlpha(outer)) - 255 * (alpha)), 0, 255);
		...
		alphaTest = MathUtil.clamp((int) ((NEX.getAlpha(inner)) - 255 * (alpha)), 0, 255);
		
		inner = ((alphaTest & 0x0ff) << 24) | ((red & 0x0ff) << 16) | ((green & 0x0ff) << 8) | (blue & 0x0ff);

		NEX.drawRectRoundedCorners(posi - 5 + 10 * percent, posY + this.getHeight() / 2 - 14, posi + width * 2 + 3.5F - 20 * percent, posY + this.getHeight() / 2 + 7, outer, 800);
		
		NEX.drawRectRoundedCorners(posi - 5 + 1F + 10 * percent, posY + this.getHeight() / 2 - 14 + 1F, posi + width * 2 + 3.5F - 1F - 20 * percent, posY + this.getHeight() / 2 + 7 - 1F, inner, 800);
		
		if(this.activated)
			NEX.drawRectRoundedCorners(-10, posY + this.getHeight() / 2 - 14, 40 * (percent), posY + this.getHeight() / 2 + 7, 0xa9505050, 800);
