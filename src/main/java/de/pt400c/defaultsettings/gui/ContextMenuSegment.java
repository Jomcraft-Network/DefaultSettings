...
public class ContextMenuSegment extends Segment {
	...
	private void openRenameClone(String text) {
		final GuiConfig gui = ((GuiConfig) this.gui);
		...
		gui.popupField.getWindow().addChild(new QuitButtonSegment(gui, 190, 5, 14, 14, new Function<ButtonSegment, Boolean>() {
			@Override
		...
		gui.popupField.getWindow().addChild(new ButtonRoundSegment(gui, 105 - 30, 75, 60, 20, "Okay", null, new Function<ButtonRoundSegment, Boolean>() {
			@Override
			public Boolean apply(ButtonRoundSegment button) {

				if(!gui.scrollableProfiles.lel.contains(ContextMenuSegment.this.nameField.query.toLowerCase())) {

				File fileDir = new File(FileUtil.getMainFolder(), ContextMenuSegment.this.nameField.tex);
				File fileDir2 = new File(FileUtil.getMainFolder(), ContextMenuSegment.this.nameField.query);
				try {
				...
				gui.scrollableProfiles.context.id = ContextMenuSegment.this.nameField.query;
				gui.popupField.setOpening(false);
				...
				ContextMenuSegment.this.setPos(-100, -100);
					...
					ContextMenuSegment.this.nameField.denied = true;
				}
				return true;
			}
		}, 0.8F, true));
