			...
			if (this.grabbed) {
				...
				if(!FileUtil.privateJson.currentProfile.equals(this.name)) {
					...
					gui.popupField.getWindow().addChild(new QuitButtonSegment(gui, 190, 5, 14, 14, new Function<ButtonSegment, Boolean>() {
						@Override
						public Boolean apply(ButtonSegment button2) {
					...
					gui.popupField.getWindow().addChild(new ButtonRoundSegment(gui, 105 - 30, 75, 60, 20, "Okay", null, new Function<ButtonRoundSegment, Boolean>() {
						@Override
						public Boolean apply(ButtonRoundSegment button2) {
			...
			if (this.grabbed) {
				if (this.isSelected(mouseX, mouseY))
					...
					gui.popupField.getWindow().addChild(new ButtonRoundSegment(gui, 105 - 30, 75, 60, 20, "Okay", null, new Function<ButtonRoundSegment, Boolean>() {
						@Override
						public Boolean apply(ButtonRoundSegment button2) {
								if (!gui.scrollableProfiles.lel.contains(AddSegment.this.nameField.query.toLowerCase())) {

									gui.scrollableProfiles.context.backgroundTimer = 2.5F * (MathUtil.PI / 3);
									gui.scrollableProfiles.context.setPos(-100, -100);

									File fileDir = new File(FileUtil.getMainFolder(), AddSegment.this.nameField.query);
									fileDir.mkdir();
									gui.scrollableProfiles.guiContentUpdate(gui.scrollableProfiles.searchbar.query);

									gui.scrollableProfiles.context.id = AddSegment.this.nameField.query;
									gui.popupField.setOpening(false);

								} else {
									AddSegment.this.nameField.denied = true;
								}
