...
public class ButtonControlSegment extends BakedSegment {
	...
	public boolean isSelected(int mouseX, int mouseY) {
		boolean general = (((DefaultSettingsGUI) this.gui).popupField == null || this.getIsPopupSegment()) && ((NEX.distanceBetweenPoints(this.getPosX() + this.getHeight() / 2F, this.getPosY() + this.getHeight() / 2F, mouseX, mouseY) <= this.getHeight() / 2F) || (NEX.distanceBetweenPoints(this.getPosX() + this.getWidth() - this.getHeight() / 2F, this.getPosY() + this.getHeight() / 2F, mouseX, mouseY) <= this.getHeight() / 2F) || (mouseX >= this.getPosX() + (height / 2F) && mouseY >= this.getPosY() && mouseX < this.getPosX() + this.getWidth() - (height / 2F) && mouseY < this.getPosY() + this.getHeight()));
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if(!compiled) {
	    	...
	    	NEX.drawRectRoundedCorners(0, 0, width, height, this.color, Integer.MAX_VALUE);
			...
			int red = (int) ((NEX.getRed(off)) + 35 * (processFactor));
			
			int green = (int) ((NEX.getGreen(off)) + 35 * (processFactor));
				
			int blue = (int) ((NEX.getBlue(off)) + 35 * (processFactor));

			NEX.drawRectRoundedCorners(diff, diff, width - diff, height - diff, ((255 & 0x0ff) << 24) | ((red & 0x0ff) << 16) | ((green & 0x0ff) << 8) | (blue & 0x0ff), Integer.MAX_VALUE);
			...
			red = (int) ((NEX.getRed(color)) + 30 * (testFactor));
			
			green = (int) ((NEX.getGreen(color)) + 30 * (testFactor));
				
			blue = (int) ((NEX.getBlue(color)) + 30 * (testFactor));

			NEX.drawRectRoundedCorners(diff, diff, width - diff, height - diff, ((255 & 0x0ff) << 24) | ((red & 0x0ff) << 16) | ((green & 0x0ff) << 8) | (blue & 0x0ff)/*on*//*selected ? 0xff606060 : 0xff3c3c3c*/, Integer.MAX_VALUE);
	    	...
	    	NEX.drawCircle(width - diff - (height - diff * 2) / 2, (height) / 2, (height - diff * 2) / 2, 270, 50);
	    	
	    	NEX.drawRect(width - diff - (height - diff * 2) / 2 - 4, diff, width - diff - (height - diff * 2) / 2, height - diff, color, false, null, false);
	    	
	    	NEX.drawRect(width - diff - (height - diff * 2) / 2 - 5, diff, width - diff - (height - diff * 2) / 2 - 4, height - diff, NEX.darkenColor(color, 0.8F).getRGB(), false, null, false);
			...
			NEX.drawScaledTex((float) this.width - 20, (float) 6, (int) (18 - alpha), (int) (18 - alpha));
