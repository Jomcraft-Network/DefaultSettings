...
import net.jomcraft.defaultsettings.gui.*;
	...
	public static class HeaderPart extends BakedSegment {
		...
		public void render(int mouseX, int mouseY, float partialTicks) {
			...
			if(!compiled) {
				...
				glEnable(GL_TEXTURE_2D);
				fontRenderer.drawString("Tab", MathUtil.clamp(72 / 2 - (fontRenderer.getStringWidth("Tab", 1.2F, true) / 2), 0, Integer.MAX_VALUE), 7, 0xffffffff, 1.4F, true);

				fontRenderer.drawString("- DefaultSettings -", 100 + (this.gui.field_230708_k_ - 100) / 2 - fontRenderer.getStringWidth("- DefaultSettings -", 1.2F, true) / 2, 8, 0xffffffff, 1.2F, true);
				
				fontRenderer.drawString(tabName, 80, 8, 0xffffffff, 1.2F, true);
