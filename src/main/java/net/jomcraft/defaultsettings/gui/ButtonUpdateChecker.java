...
import net.jomcraft.defaultsettings.UpdateContainer;
import net.jomcraft.defaultsettings.UpdateContainer.Status;
...
public class ButtonUpdateChecker extends ButtonSegment {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
    	...
    	NEX.drawRectRoundedCorners(this.getPosX(), this.getPosY(), this.getPosX() + this.getWidth(), this.getPosY() + this.getHeight(), NEX.darkenColor(statusToColor(DefaultSettings.getUpdater().getStatus(), darken), 0.5F).getRGB(), 5);
	
		float factor = 1F - ((5 - inRad) / 5);
	       
	    float innerRadius = 5 - (factor * 5);

	    NEX.drawRectRoundedCorners(this.getPosX() + inRad, this.getPosY() + inRad, (float) this.getPosX() + this.width - inRad, (float) this.getPosY() + this.height - inRad, statusToColor(DefaultSettings.getUpdater().getStatus(), darken), innerRadius < 0 ? 0 : innerRadius);
