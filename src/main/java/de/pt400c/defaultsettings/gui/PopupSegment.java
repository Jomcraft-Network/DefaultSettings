...
public class PopupSegment extends Segment {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		if(resized != this.resized_mark) {
			...
			bufferWidth = testWidth * scaledresolution.getScaleFactor();
			bufferHeight = testHeight * scaledresolution.getScaleFactor();
			this.popX = this.gui.width / 2 - (testWidth - 10) / 2;
			this.popY = this.gui.height / 2 - (testHeight - 10) / 2;
		...
		if (this.isVisible) {
			...
			if (!compiled) {
				...
				glViewport((int) 0, (int) 0, (int) MC.displayWidth, (int) MC.displayHeight);
				compiled = true;
			}
			...
			if (this.dragging) {
				...
				if(!((this.popX - origX) == 0 && (this.popY - origY) == 0)) {
					for(Segment segment : this.window.children) {
						segment.setPosHit(segment.posX, segment.posY, PopupSegment.this.popX, PopupSegment.this.popY);
					}
				}
