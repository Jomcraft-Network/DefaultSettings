...
public class FileUtil_18 {
	...
	public static void restoreContents() throws NullPointerException, IOException {
		...
		if (!DefaultSettings_18.VERSION.equals(version))
			mainJson.setVersion(DefaultSettings_18.VERSION).setPrevVersion(version);
		...
		if (keysFile.exists()) {
			...
			try {
				...
				while ((line = reader.readLine()) != null) {
					...
					DefaultSettings_18.keyRebinds.put(line.split(":")[0], new KeyContainer_18(Integer.parseInt(line.split(":")[1]), line.split(":").length > 2 ? KeyModifier.valueFromString(line.split(":")[2]) : KeyModifier.NONE));
				}
			} catch (IOException e) {
				throw e;
			...
			if (update) {
				...
				for (KeyBinding keyBinding : Minecraft.getMinecraft().gameSettings.keyBindings) {
					if (DefaultSettings_18.keyRebinds.containsKey(keyBinding.getKeyDescription())) {
						KeyContainer_18 container = DefaultSettings_18.keyRebinds.get(keyBinding.getKeyDescription());
